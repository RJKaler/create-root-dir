...
#This is a basic script that will create user friendly directories for Ubuntu or other distros (these are the same directories that come as default for regular users). To make this work, you first have to enable root. You can do that simply with sudo passwd root. Root will come with default sys files like /etc/ /sys /mnt 
